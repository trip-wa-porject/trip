...
export default function filter(filters: Partial<TripFilter>, data: Trip) {
  return Object.keys(filters).every((e) => {
    switch (e) {
      ...
      case 'level': {
        return filters?.level ? filters.level.includes(data.level) : true;
      }
      case 'type': {
        return filters?.type ? filters.type.includes(data.type) : true;
      }
      case 'region': {
        return filters?.region
          ? filters?.region
            ? data.area.some((e) => {
                return filters?.region?.includes(e.city);
              })
            : true
          : true;
      }
